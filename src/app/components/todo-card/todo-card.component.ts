...
export class TodoCardComponent implements OnInit {
  ...
  private updateCategories(): void {
    this.categories.set(
      Array.from(new Set(this.todoList().map(todo => todo.category).filter(category => category)))
    );
  }
  ...
  public filteredTodoList = computed(() => {
    ...
    return this.todoList().filter(todo => 
      !selectedCategory || todo.category.toLowerCase() === selectedCategory
    );
  });

  public filterTodos(): void {
    // This method is triggered by the selectionChange event of the mat-select
    // It will automatically update the filteredTodoList due to the computed property
  }
